...
class ActionItemView: UIView {
    ...
    fileprivate(set) var iconWidth: CGFloat = 0
    ...
    convenience init(action: SwipeableCellAction, index: Int) {
        ...
        self.iconWidth = action.iconWidth
        ...
        if let image = image, let title = title {
            ...
            addConstraint(NSLayoutConstraint(item: imageView, attribute: .width, relatedBy: .equal, toItem: .none, attribute: .notAnAttribute, multiplier: 1, constant: self.iconWidth))
            addConstraint(NSLayoutConstraint(item: imageView, attribute: .height, relatedBy: .equal, toItem: .none, attribute: .notAnAttribute, multiplier: 1, constant: self.iconWidth))
...
class SwipeableCellActionsView: UIView {
    ...
    convenience init(actions: [SwipeableCellAction]?, parentCell: TimelineTableViewCell) {
        self.init(frame: .zero)
        translatesAutoresizingMaskIntoConstraints = false
